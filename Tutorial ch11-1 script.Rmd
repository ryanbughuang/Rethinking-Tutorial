...
When $theta$ = 2, $\bar{p}$ is distributed approximately uniformly except for the tails.
```{r}
...
The over-dispersion problem can also be solved by constructing a multilevel model. With that, we can assign our own *prior* distribution on the *prior*. The higher level prior is called **hyper priors**. By comparing the parameters of hyper priors of different genders, we will have similar result of beta-binomial/gamma-poisson model. We will learn about the detail in tutorial-ch13.
