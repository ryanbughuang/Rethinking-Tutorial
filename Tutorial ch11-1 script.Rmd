...
### Multilevel Model
CH12 Multilevel Model in ver1.
CH13 Models without Amnesia in ver2.
...
Calculate the log likelihood is straightfoward, we just take the log on $Pr(y = 0|p_{drink}, \lambda)$ and $Pr(y > 0|p_{drink}, \lambda)$. 

The result (in Stan syntax):
$y = 0: log(p) + log(1 - p) * PoissonLpmf(0|\lambda)$
$y > 0: log(1 - p) * PoissonLpmf(y|\lambda)$

Let's simulate a fake data
```{r}
# define parameters
...
* if statement: there are two siturations in the 
```{r}
m11.2.1 = "
data {
	int N;
