...
The predictions for an individual one illustrate how variable different chimpanzees react to the experiment conditions.
...
###Apply Method
```{r}
#recall: post = extract.samples(fit12.2.2c, pars=fit12.2.2c@model_pars[1:7])
pbar_link = function( treatment ) {
  logodds <- with( post , alpha_bar + beta[,treatment]*sigma_beta ) 
  return( inv_logit(logodds) )
}
...
psim_link = function( treatment ) {
  logodds <- with( post , a_sim + beta[,treatment]*sigma_beta ) 
  return( inv_logit(logodds) )
}
